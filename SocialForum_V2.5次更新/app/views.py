...
def newarticle(c):
    ...
    return render_template('newarticle.html', title = 'newarticle',form = form)
'''
@app.route('/updatearticle/<int:u>', methods = ['GET', 'POST'])
@login_required
def updatearticle(u):
    form = ArticleForm()
    art=Article.query.filter_by(id=u).first()
    form.data['title']=art.title
    form.data['body']=art.body
    if request.method == 'POST':
        if form.validate_on_submit():
            title=form.data['title']
    	    body = form.data['body']
    	    type_a=art.type_a
    	    author=art.user_id
    	    a=Article(id=art.id,title=title,body=body,type_a=type_a,timestamp=datetime.datetime.now(),author=author)
    	    try:
	        ...
	        flash(u'修改失败')
        return redirect('/article/'+str(u))
    return render_template('updatearticle.html', title = 'updatearticle',form = form)
'''
